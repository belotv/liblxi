...
#define MDNS_MULTICAST_RESPONSE 0x0000U
...
static inline int
mdns_socket_open_ipv4(const struct sockaddr_in* saddr, char* interface);
...
//! and queries, you must set MDNS_PORT as port.
static inline int
mdns_socket_setup_ipv4(int sock, const struct sockaddr_in* saddr, char* interface);
...
//! and queries, you must set MDNS_PORT as port.
static inline int
mdns_socket_open_ipv6(const struct sockaddr_in6* saddr, char* interface);
...
//! and queries, you must set MDNS_PORT as port.
static inline int
mdns_socket_setup_ipv6(int sock, const struct sockaddr_in6* saddr, char* interface);
...
mdns_socket_open_ipv4(const struct sockaddr_in* saddr, char* interface) {
    ...
    if (sock < 0)
        return -1;
    if (mdns_socket_setup_ipv4(sock, saddr, interface)) {
...
mdns_socket_open_ipv6(const struct sockaddr_in6* saddr, char* interface) {
    ...
    if (sock < 0)
        return -1;
    if (mdns_socket_setup_ipv6(sock, saddr, interface)) {
        mdns_socket_close(sock);
        return -1;
    }
    return sock;
...
#ifdef __APPLE__
    int interface_idx = if_nametoindex(interface);
    if (setsockopt(sock, IPPROTO_IPV6, IPV6_BOUND_IF, &interface_idx, sizeof(interface_idx)))
        return -1;
#else
...
static const uint8_t mdns_services_query[] = {
    ...
    // QM (multicast response) and class IN
    0x00, MDNS_CLASS_IN};
